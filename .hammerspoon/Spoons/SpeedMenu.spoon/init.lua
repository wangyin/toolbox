...
local function data_diff()
    ...
    if obj.darkmode then
        obj.disp_str = hs.styledtext.new(disp_str, {font={size=9}, color={hex="#FFFFFF"}, paragraphStyle={alignment="left", maximumLineHeight=18}})
    else
        obj.disp_str = hs.styledtext.new(disp_str, {font={size=9}, color={hex="#000000"}, paragraphStyle={alignment="left", maximumLineHeight=18}})
    end
    local canvas = hs.canvas.new{x = 0, y = 0, h = 24, w = 70}
    canvas[1] = {type = 'text', text = obj.disp_str}
    obj.menubar:setIcon(canvas:imageFromCanvas())
    canvas:delete()
    canvas = nil
